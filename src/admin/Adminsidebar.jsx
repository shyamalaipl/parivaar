...
const Adminsidebar = ({ setActivePage, activePage, isOpen, setIsOpen }) => {
  const menuItems = [
    // ... (your menuItems array remains the same)
    { id: "admindashboard", name: "Dashboard", icon: <Home size={22} /> },
    ...
    // { id: "userrequist", name: "User Requests", icon: <FileText size={22} /> },
    // { id: "userrejected", name: "User Rejected", icon: <FileText size={22} /> },
    {
      id: "admineventlist",
      name: "Events",
    ...
    { id: "profileadmin", name: "Profile", icon: <User size={22} /> },

    // { id: "settings", name: "Settings", icon: <Settings size={22} /> },
    // { id: "designationCreation", name: "DesignationCreation", icon: <Settings size={22} /> },
  ...
  return (
    ...
    >
      {/* Fixed Header */}
      <div className="p-4 border-b border-gray-700/50 flex items-center justify-between w-full sticky top-0 z-10 bg-[#1A2434]">
        {isOpen ? (
          <>
        ...
        {menuItems.map((item) => (
          <button
            ...
            className={`w-full flex items-center px-4 py-3 rounded-lg transition-all duration-300 group relative
              ${
                activePage == item.id
                  ? "bg-[#252F3F]  text-[white] shadow-md"
      ...
      {/* Fixed Footer */}
      <div className="p-4 border-t border-gray-700/50 w-full sticky bottom-0 bg-[#1A2434]">
        <Link to="/">
          <button
            onClick={clearLocalStorage}
            className="w-full flex items-center px- py-3 text-red-500 hover:bg-red-900/20 hover:text-red-800 rounded-lg transition duration-300 group relative"
          >
