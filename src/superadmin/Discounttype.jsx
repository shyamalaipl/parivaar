...
const Discounttype = () => {
  ...
  const fetchDiscountTypes = async () => {
    ...
    try {
      const response = await fetch(
        "https://parivaar.app/public/api/discount_types"
      );
  ...
  // Create or Update discount type
  const handleSubmit = async (e) => {
    ...
    try {
      const url = isEditMode
        ? `https://parivaar.app/public/api/discount_types/${currentId}`
        : "https://parivaar.app/public/api/discount_types";
      ...
      const response = await fetch(url, {
        ...
        body: JSON.stringify({ Type_Name: formData.type }),
      });
  ...
  // Edit discount type
  const handleEdit = (id) => {
    const discountTypeToEdit = discountTypes.find((item) => item.DT_Id == id);
    if (discountTypeToEdit) {
      setFormData({ type: discountTypeToEdit.Type_Name });
      setIsEditMode(true);
      setCurrentId(id);
      setIsModalOpen(true);
    }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetch(
        `https://parivaar.app/public/api/discount_types/${id}`,
        {
          method: "DELETE",
        }
      );
      ...
      setError("Failed to delete discount type");
    } finally {
      setIsLoading(false);
    }
  };
          ...
          <div className="bg-white rounded-xl p-6 w-full max-w-md">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-xl font-bold text-[#172030]">
                {isEditMode ? "Edit Discount Type" : "Add Discount Type"}
              </h3>
              <button
                onClick={() => {
                  setIsModalOpen(false);
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="Enter discount type (e.g., Quantity, Amount)"
                  required
                  disabled={isLoading}
                />
              </div>
